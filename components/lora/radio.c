...
uint8_t lora_drv_init = 0;
static SemaphoreHandle_t lora_irq_sem = NULL; // 中断信号量
...
void lora_task_handle(void *pvParameters)
{
	...
	while(1)
	{
		if(xSemaphoreTake(lora_irq_sem, portMAX_DELAY))
		{
			uint16_t irqRegs = GetIrqStatus();
			ClearIrqStatus(SX126X_IRQ_ALL); // 清除所有中断标志
			// 处理接收完成事件
        	if( ( irqRegs & SX126X_IRQ_RX_DONE ) == SX126X_IRQ_RX_DONE )
			{
				if( ( irqRegs & SX126X_IRQ_CRC_ERR ) == SX126X_IRQ_CRC_ERR )
				{    
					ESP_LOGI(pcTaskGetName(NULL), "lora recv crc error");
				}
				else
				{
                	uint8_t rxLen = ReadBuffer(buf, sizeof(buf));
			...
			// 处理发送完成事件
            if (irqRegs & (SX126X_IRQ_TX_DONE | SX126X_IRQ_TIMEOUT)) {
                if (txActive) 
				{
                    txActive = false;
...
void radio_init(void)
{
    ...
    gpio_set_intr_type(SX126x_DIO1, GPIO_INTR_POSEDGE);

	// 创建中断信号量
    lora_irq_sem = xSemaphoreCreateBinary();

    gpio_install_isr_service(0);
    gpio_isr_handler_add(SX126x_DIO1, dio1_isr_handler, NULL);
	...
	uint32_t frequencyInHz = 434000000;
...
void LoRaConfig(uint8_t spreadingFactor, uint8_t bandwidth, uint8_t codingRate, uint16_t preambleLength, uint8_t payloadLen, bool crcOn, bool invertIrq) 
{
	...
	SetDioIrqParams(SX126X_IRQ_ALL, //all interrupts enabled
		SX126X_IRQ_RX_DONE | SX126X_IRQ_TX_DONE | SX126X_IRQ_TIMEOUT | SX126X_IRQ_CRC_ERR, //interrupts on DIO1
...
bool LoRaSend(uint8_t *pData, int16_t len, uint8_t mode)
{
	uint16_t irqStatus;
	bool rv = false;
	if ( lora_drv_init == true && txActive == false )
	{
		txActive = true;
